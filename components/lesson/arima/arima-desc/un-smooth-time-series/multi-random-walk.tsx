    ...
    const handleGen = useMemo(
        () =>
            catchErrorWrapper(async () => {
                const res = await dataPoster('desc/5', {
                    nwalks: nWalks,
                    nsteps: nStep,
                });
                setRes(res);
    ...
    return (
        <div>
            ...
            <div className="border-y py-2">
                <CapTitle className="my-2" index={1} title="生成随机整数数组"
                          tip="生成一个大小为[次数, 步数]的随机整数数组，每个元素取值为0或1"/>
                ...
                {stepData.length ? (
                    <>
                        ...
                        />
                        <CapTitle className="my-2" index={3} title="累加生成游走序列"
                                  tip="对每一次游走的步长进行累积求和"/>
